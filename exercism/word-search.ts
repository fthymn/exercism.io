...
export class WordSearch {
  ...
  prepareDiagonalGrid() {
    ...
    for (let line = 1; line <= row_count + col_count - 1; line++) {
      let start_col = Math.max(0, line - row_count);
      let count = Math.min(line, col_count - start_col, row_count);
      let str: string = '';
      let strOpposite: string = '';
      let gridItem: Grid = new Grid();
      let gridItemOpposite: Grid = new Grid();
      for (let j = 0; j < count; j++) {
        str += matrix[Math.min(row_count, line) - j - 1][start_col + j];
        gridItem.locations.push(<Location>{
          x: start_col + j + 1,
          y: Math.min(row_count, line) - j,
        });
        if (matrix.length > Math.min(col_count, line) - j - 1) {
          strOpposite +=
            matrix[Math.min(col_count, line) - j - 1][
              col_count - ((start_col + j) % col_count) - 1
            ];
          gridItemOpposite.locations.push(<Location>{
            x: col_count - ((start_col + j) % col_count),
            y: Math.min(col_count, line) - j,
          });
        }
      }
      gridItem.word = str;
